...
for row in csv.DictReader(open("/dropbox/LEGO/inventory/inventory.tsv"), dialect='excel-tab'):
  cursor.execute("insert into part_transactions (part_num, color_id, quantity) values (?, (select id from colors where name=?), ?)", (row['id'], row['color'], row['quantity']))
conn.commit()
conn.close()
...
for part in part_replacements:
  helpers.query("update part_transactions set part_num=? where part_num=?", (part[1], part[0]))