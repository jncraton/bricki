...
for part in list(cursor.fetchall()):
  cursor.execute("select part_num from part_details where part_num=? or bricklink_id=? or ldraw_id=? or lego_id=?", (part[0], part[0], part[0], part[0]))
  try:
    new_part_num = cursor.fetchall()[-1][0]
    print("Updating %s to %s" % (part[0], new_part_num))
    cursor.execute("update part_transactions set part_num=? where part_num=?", (new_part_num, part[0]))
  except IndexError:
